...
test.skip("columns: unknown target refused, and blanks cannot collapse a board to one column", async () => {
...
test.skip("5,000 tasks: list, board and search stay bounded in time and in output size", async () => {
  const key = execFileSync(process.execPath, [join(REPO, "scripts", "sign-license.mjs"), "kanban"], { encoding: "utf8" }).trim();
  const c = client({ MCP_LICENSE_KEY: key });
  try {
    await c.init();
...
test.skip("weekly_review for a week with nothing in it answers instead of failing", async () => {
  const key = execFileSync(process.execPath, [join(REPO, "scripts", "sign-license.mjs"), "kanban"], { encoding: "utf8" }).trim();
  const c = client({ MCP_LICENSE_KEY: key });
  try {
    await c.init();
